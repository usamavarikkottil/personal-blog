...
1. Open the file /themes/`<your_theme_name`/exampleSite/`config.toml`.
   In our case it is /themes/devise/exampleSite/`config.toml`.
2. Copy the contents inside the file and paste it in our web site's config.toml. That is the file /`config.toml`
   

   ![change hugo website title](/images/how-i-created-my-hugo-website/16-change-website-config-toml-file.png)
   
   ---

3. Change the configurations as our like. We can change details such as Website title, menus, home images, social links, etc.
4. Now save the file and enter below commands on the terminal session again.

```bash
git add --all
git commit -m "change website configurations"
git push -u origin master
```
---


##### Changes made are highlighted in this image.



![changed hugo website look](/images/how-i-created-my-hugo-website/17-final-look.png)

---

#### Conclusion

We have made our first website in hugo. As we get used to hugo, it would become much more easier to manage website. We have covered from installing Hugo to Making our hugo website live. Hugo is such a simple, fastest framework. And it is simply awesome.